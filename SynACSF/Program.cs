    ...
    internal class Program
    {
        ...
        public static SkillTree ReadConfigFile(IPatcherState<ISkyrimMod, ISkyrimModGetter> state, string Path)
        {
            ...
            if (cv.Entries?.GetValueOrDefault("LevelFile") != "")
            {
                ...
                tree.StartingLevel = state.LinkCache.Resolve<IGlobalFloatGetter>(new FormKey(ModKey.FromFileName(cv.Entries?.GetValueOrDefault("LevelFile") ?? ""), uint.Parse(cv.Entries?.GetValueOrDefault("LevelId")?.Substring(2) ?? "")))?.Data.ToString()??"0";
            }
